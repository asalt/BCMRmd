...
.sparkline <- df %>% filter(ScanType==1) %>%  
  group_by(id) %>%
  arrange(ScanNumber) %>%
  summarise(TotalIonIntensity=spk_chr(TotalIonIntensity, width=60, height=10))

#.sparkline <- dfp %>% group_by(SpectrumFile) %>% arrange(ParentIonIndex) %>%   summarise(value=spk_chr(ParentIonIntensity, options=list(spotColor=FALSE)))
# kable(test, escape=F)
.sparkline %>% formattable::format_table(
  align = c("l", "r"), 
  # SpectrumFile = formattable::style(align='l'),
