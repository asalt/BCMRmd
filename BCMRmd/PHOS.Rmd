...
phos_gct_log %>% write_gct(.file, precision = 8)
.file <- file.path(ROOTPATH, "MSPC000936_phos_log_batchcor.tsv")
readr::write_tsv(mat(phos_gct_log) %>% as.data.frame() %>% rownames_to_column("id"), .file)
...
make_heatmap <- function(.gct, show_row_names=F, cut_by=NULL, pos_sel=NULL, cluster_rows=T, cluster_columns=T, ...){
  .more_params = list(...)
...
make_p_table <- function(geneids){
  prof_gids <- intersect(geneids, prof_gct_log@rid)
  prof_expression_data <- mat(prof_gct_log)[prof_gids,]
  
  phos_gids <- intersect(geneids, phos_gct_log@rdesc$GeneID)
  .psites <- phos_gct_log@rdesc[phos_gct_log@rdesc$GeneID %in% geneids, ]
  phos_expression_data <- mat(phos_gct_log)[rownames(.psites), ]
...
limma_res_phos %<>%  pivot_wider(id_cols = GeneSymbolSite, names_from = contrast,
                               values_from = c("logFC", "AveExpr", "t", "P.Value", "adj.P.Val", "B")
                               )

limma_res_phos_meta <- limma_res_phos %>% left_join(phos_gct@rdesc,
...
.front <- intersect(.order, colnames(limma_res_phos_meta))

# Reorder the dataframe
limma_res_phos_meta <- limma_res_phos_meta %>%
...
limma_res_phos_meta %>% write_tsv(.file)

```

```{r, label='phos.limma.prof.run', eval=T}
fit <- do_limma(prof_gct_log)
...
phos_prof_stats <- left_join(limma_res_phos,
          limma_res_prof,
...
```{r, label='phos.2d.differential', eval=T, fig.width=10, fig.height=12}
phos_prof_stats %>% 
  ggpubr::ggscatter(
    x="logFC.phos",
    y="logFC.prof",
...
plot_contrast_top50 <- function(contrast){
  ...
  .sel <- phos_prof_stats %>% 
    ...
    head(50)
  geneids <- .sel$GeneID
  .rids <- phos_gct_log@rdesc %>% filter(GeneID %in% geneids) %>% pull(id)
  ...
  .gct %>% make_heatmap(pos_sel = pos_sel, cut_by = cut_by, show_row_names = T, column_title=contrast)
}
...
```{r, label=phos.geneset.heatmap, fig.width=15, fig.height=7}
