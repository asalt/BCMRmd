...
#if 
#this is not needed
#batchinfo <- phos_gct_log@cdesc %>% mutate(id=factor(id, levels=colnames(mat_log), ordered=TRUE)) %>% arrange(id)
batchinfo <- batchinfo[[batchvar]] %>% as.character()
mat_combat <- sva::ComBat(phos_gct_log@mat, batchvar)
phos_gct_batch <- phos_gct %>% cmapR::GCT(mat = mat_combat, rid = .@rid, cid = .@cid, cdesc = .@cdesc, rdesc = .@rdesc)
...
mat_combat <- sva::ComBat(prof_gct_log@mat, batchvar)
...
```{r, label="phos-pca-make-prof", echo=T, eval=T}
...
```
LINK
```{r, label="pca-report-1-html-pagetest", echo=F, results="asis", eval=T}
.html <- cat(paste0("[", .out, "](", .out, ")"), "\n\n")
# print(.html)
## {-}
```

```{r, label="phos-pca-make-phos", echo=T, eval=T}
gct <- phos_gct_log
...
LINK
```{r, label="pca-report-2-html-pagetest", echo=F, results="asis", eval=T}
.html <- cat(paste0("[", .out, "](", .out, ")"), "\n\n")
# print(.html)
## {-}
```
...
formula <- get_param(subparams$formula)
contrasts <- get_param(subparams$contrasts)
